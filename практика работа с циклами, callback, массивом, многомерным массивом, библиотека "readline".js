...
// UOD: ПРАКТИКА РАБОТЫ С CALLBACK:
...
myCallbackFunction.addMyCallback("Операции с массивом:\n1 - работа с массивом\n2 - завершить операцию\n\nВыберите опцию: ", callback => {
    switch(+callback) {
        case 1: 
            for (let rowIndex = 0; rowIndex < namesArray.length; rowIndex++) {
                console.log(`Строка: ${rowIndex + 1}: `);
            ...
            myCallbackFunction.addMyCallback("\nВыберите строку массива:\n1 - первая строка\n2 - вторая строка\nВыберите действие: \n", rowSelection => {
                for (let columnIndex = 0; columnIndex < namesArray[rowSelection - 1].length; columnIndex++) {
                    console.log(`Индекс: ${columnIndex + 1} - ${namesArray[rowSelection - 1][columnIndex]}`);
                }
                ...
                myCallbackFunction.addMyCallback("\nИндекс который хотите изменить: \n", index => {
                    myCallbackFunction.addMyCallback("\nВыберите действие с индексом:\n1 - удалить\n2 - изменить\nВыберите действие: \n", callback => {
                        switch (+callback) {
                            ...
                            case 2:
                                myCallbackFunction.addMyCallback("Введите текст: ", edit => {
                                    selectedRow[index -1] = edit;
                                    console.log(`Значение в массиве изменено на: ${selectedRow[index -1]}. ${selectedRow}`)
                                })
                            break;
                        }
                    })
